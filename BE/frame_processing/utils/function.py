...
def select_keyframes(video_path:str, thres:float, output_path=str, step=10):
    ...
    for frame_idx in tqdm(range(0, frame_count), desc="Processing video frames"):
        ...
        if frame_idx % step == 0:
            ...
            if K is None:
                save_keyframe(fr_i, frame_idx, output_path)
                v_prev = v_i
                K = v_i
                continue
...
def save_keyframe(keyframe ,idx, output_folder_path):
    if not os.path.exists(output_folder_path):
        os.makedirs(output_folder_path)
    frame_filename = os.path.join(output_folder_path, f"keyframe_{idx:04d}.png")
